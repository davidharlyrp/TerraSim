...
/// Quad9 elements (9 nodes, 9 Gauss points, 3×3 Gauss–Legendre).

use numpy::ndarray::{Array1, Array2, Array3};
use numpy::{PyArray1, PyArray2, PyArray3, PyReadonlyArray1, PyReadonlyArray2, PyReadonlyArray3};
use pyo3::prelude::*;

use crate::elements::quad9;
const NUM_GP: usize = quad9::NUM_GAUSS;
const NUM_NODES: usize = quad9::NUM_NODES;
...
pub fn compute_stresses_loop_py<'py>(
    ...
    b_matrices: PyReadonlyArray2<'py, f64>,          // (N*9, 54) flattened B per GP
    det_j: PyReadonlyArray2<'py, f64>,               // (N, 9)
    weights: PyReadonlyArray1<'py, f64>,             // (9,)
    ...
    let mut new_stresses = Array3::<f64>::zeros((num_active, NUM_GP, 3));
    ...
    for i in 0..num_active {
        ...
        for gp_idx in 0..NUM_GP {
            let b_row = i * NUM_GP + gp_idx;
            let mut b_gp = [0.0f64; 54];
            for k in 0..54 {
            ...
            let epsilon_total = quad9::b_times_u(&b_gp, &u_el);
            let start_str = [
            ...
            // BT @ sig_new
            let bt_sig = quad9::bt_times_sigma(&b_gp, &sig_new);
...
/// Stiffness assembly kernel — Quad9 elements (18×18 per element).
#[pyfunction]
#[pyo3(name = "assemble_stiffness_loop")]
pub fn assemble_stiffness_loop_py<'py>(
    py: Python<'py>,
    d_tangent: PyReadonlyArray2<'py, f64>,  // (N*9, 9) flattened D per GP
    b_matrices: PyReadonlyArray2<'py, f64>, // (N*9, 54) flattened B per GP
    det_j: PyReadonlyArray2<'py, f64>,      // (N, 9)
    weights: PyReadonlyArray1<'py, f64>,    // (9,)
    ...
    let mut k_values = Array1::<f64>::zeros(num_active * 324);

    for i in 0..num_active {
        let mut k_el = [0.0f64; 324];
        for gp_idx in 0..NUM_GP {
            let row = i * NUM_GP + gp_idx;
            ...
            let contrib = quad9::btdb_contribution(&b, &d, det_j_arr[[i, gp_idx]], w_arr[gp_idx]);
            for k in 0..324 {
                k_el[k] += contrib[k];
            }
        }

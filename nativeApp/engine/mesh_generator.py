...
def _min_dist_to_attractors(
    point: np.ndarray,
    attractor_points: List[List[float]] | None,
    attractor_segments: List[Tuple[List[float], List[float]]] | None,
) -> float:
    """Compute minimum distance from a point to the nearest attractor point or segment."""
    min_d = float('inf')
    if attractor_points:
        for ap in attractor_points:
            d = float(np.linalg.norm(point - np.array(ap)))
            if d < min_d:
                min_d = d
    if attractor_segments:
        for seg_a, seg_b in attractor_segments:
            d = _point_to_segment_dist(point, np.array(seg_a), np.array(seg_b))
            if d < min_d:
                min_d = d
    return min_d
...
def _apply_even_transfinite_all_curves(
    all_lines,
    mesh_size: float,
    skip_tags: set[int] | None = None,
    attractor_points: List[List[float]] | None = None,
    attractor_segments: List[Tuple[List[float], List[float]]] | None = None,
    ...
    When attractor data is provided, curves near attractors use a smaller local
    mesh size (matching the Threshold field grading) so boundary element counts
    are consistent with the interior refinement.
    """
    skip = skip_tags or set()
    has_attractors = bool(attractor_points or attractor_segments)
    h_min = mesh_size * h_min_ratio
    # Tight refinement zone: 5x mesh_size from any attractor
    dist_max = mesh_size * 5.0
    ...
    for _dim, line_tag in all_lines:
        ...
        local_h = mesh_size
        if has_attractors:
            # Sample the curve midpoint to estimate distance to nearest attractor
            try:
                bnd = gmsh.model.getBoundary([(1, line_tag)], combined=False, oriented=False)
                ...
                mid = None
            if mid is not None:
                d = _min_dist_to_attractors(mid, attractor_points, attractor_segments)
                if d <= dist_min:
                    local_h = h_min
                elif d < dist_max:
                    t = (d - dist_min) / (dist_max - dist_min)
                    local_h = h_min + t * (mesh_size - h_min)
                # else: local_h stays at mesh_size

        n_elem = _even_element_count(length, local_h)
        n_points = n_elem + 1
        try:
...
def _setup_attractor_field(
    ...
    # Tight refinement zone: 5x element size from any attractor
    dist_max = max_element_size * 5.0
...
def _run_gmsh_quad9(
    ...
    try:
        ...
        # Collect attractor data BEFORE applying transfinite so curves near
        # loads/EBR get finer element counts that match the background field.
        has_loads = load_refine and (nodal_forces or line_loads_gmsh)
        has_ebr = ebr_refine and bool(ebr_line_segments)

        attr_pt_coords: List[List[float]] | None = None
        attr_seg_list: List[Tuple[List[float], List[float]]] | None = None

        if has_loads or has_ebr:
            attr_pt_coords = list(load_point_coords)
            attr_seg_list = list(load_line_segments)
            for seg in ebr_line_segments:
                attr_pt_coords.extend([seg[0], seg[1]])
                attr_seg_list.append(seg)

        # All remaining 1D curves must have an even division count for Quad9.
        # Pass attractor data so curves near loads/EBR get proportionally
        # more elements, consistent with the background mesh field grading.
        _apply_even_transfinite_all_curves(
            all_lines,
            global_mesh_size,
            skip_tags=configured_curves,
            attractor_points=attr_pt_coords,
            attractor_segments=attr_seg_list,
        )

        if has_loads or has_ebr:
            attractor_pts = _find_point_tags(attr_pt_coords)
            attractor_curves = _find_curve_tags_on_segments(attr_seg_list)
